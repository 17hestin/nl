...
int ack_array[2][4];
int window_size = 4,a_get=0,r_send=0,w_pos=0,f_number=0,length=0;
...
sent_bytes = sendto(sockfd, message, strlen(message), 0, (const
...
received_bytes = recvfrom(sockfd, ack, BUFFER_SIZE, 0, (struct
...
{
printf("Acknowledgement for frame %d is not received resending window\n",ack_array[0][i]);
f_number = w_pos;
send_window(message);
}
}
